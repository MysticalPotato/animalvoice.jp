...
<div class="main">
	<div id="applications" class="content">
		<a href="<?= route('/admin/applications') ?>" class="back-btn"><- <?= __('nav.back') ?></a>
		
		<h2><?= displayName(htmlspecialchars($application['first_name']), htmlspecialchars($application['last_name'])) ?></h2>
		...
		<div class="btns">
			<?php if(!$approved) : ?>
				<form method="POST" action="<?= route('/admin/groups') ?>">
					...
					<button class="btn good-btn"><?= __('button.approve') ?></button>
				</form>
			<?php endif; ?>
			...
			<form method="POST" action="<?= route("/admin/applications/{$application['id']}") ?>">
				...
				<?php else: ?>
					<button class="btn bad-btn"><?= __('button.reject') ?></button>
				<?php endif; ?>
			</form>
		</div>
	</div>
