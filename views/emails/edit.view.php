...
<div class="content">
	...
	<form id="<?= $form_id = uniqid() ?>" method="POST" action="<?= route("/admin/emails/{$email['id']}") ?>" enctype="multipart/form-data">
        <div class="form-section">
			...
			<select name="recipient" required>
				<?php foreach($recipients as $key => $item) : 
					...
					if(old('recipient', $email['recipient']) === $key) : ?>
        ...
        <div class="form-section">
			...
			<input type="text" name="subject" value="<?= old('subject', htmlspecialchars($email['subject'])) ?>" maxlength="50" required>
		...
		<div class="form-section">
			...
			<?php // include rich text editor
				$content = old('content', $email['content']);
